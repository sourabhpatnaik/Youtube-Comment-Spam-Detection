...
# ── Master Layout Stylesheet (Escaped with double curly braces) ──
st.markdown(f"""
<style>
@import url('https://fonts.googleapis.com/css2?family=Inter:wght=400;500;600;700&display=swap');
html, body, [class*="css"] {{ font-family: 'Inter', sans-serif; }}

/* ── HIGH-END GRAPHIC VIEWPORT BACKGROUND ANCHOR ── */
.stApp {{ 
...
/* ── COMPLETE FROSTED GLASS NAVIGATION BAR UPGRADE ── */
...
/* Styled Nav Block Items inside the Frosted Sidebar */
...
/* ── FIXED ADVANCED RADIO SELECTION NUKE SWEEP ── */
/* Targets specific radio layout components, circles, dots, and containers without affecting main text scripts */
...
/* Strips any left inline positioning constraints from native stream layouts */
[data-testid="stSidebar"] div[role="radiogroup"] label [class*="st-"] {{
    margin-left: 0 !important;
    padding-left: 0 !important;
}}

/* Re-align text perfectly flush inside the block layout container */
...
/* ── DOCUMENTATION SPECIFIC SUB-STYLES ── */
...
/* ── DATASET SECTION CUSTOM TABLE STYLING ── */
...
def load_model():
    try:
        ...
        return {'model': DummyModel(), 'vectorizer': DummyVectorizer()}
...
if page_selection == "🏠  Home / Predict":
    ...
    # Input Form Container Card Layout
    with st.container():
    ...
    # Output Elements Generation Block 
    result_alert = ""
    if predict_clicked and comment.strip():
        pred = knn_model.predict(vectorizer.transform([clean_text(comment)]))[0]
        # FIX: Swapped "fill" evaluation rule back to "pred" lookup matrix configuration tracking
        if pred == 0:
